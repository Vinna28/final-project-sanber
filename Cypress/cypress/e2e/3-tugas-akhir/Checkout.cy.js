...
describe("Checkout Page", () => {
  ...
  it("Verifikasi dapat memilih dan menggunakan semua metode pengiriman yang tercantum dengan benar", () => {
    ...
    cy.get('.ship-via').should('contain', 'Flat Rate - Fixed').should('be.visible');
  });
  ...
  it("Verifikasi dapat menemukan dan memesan dalam riwayat pesanan pelanggan setelah melakukan pemesanan", () => {
    ...
    cy.get('.checkout-methods-items > :nth-child(1) > .action').click();
    cy.wait(1000)
    cy.get('#shipping > .step-title').should('be.visible').should('contain', 'Shipping Address');
    cy.wait(1000)
  ...
  it("Verifikasi apakah faktur dibuat dan dikirim ke pelanggan setelah melakukan pemesanan", () => {
    ...
    cy.get('.checkout-success').should('be.visible').should('contain', 'Your order number is');
  });
